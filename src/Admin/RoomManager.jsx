...
import { getAdRooms, uploadRoomImage, roomEdit, deleteRoom } from '../user';
...
export default function RoomManager() {
    ...
    async function handleDelete(room) {
        setErrorallRooms('')
        if (!window.confirm(`Are you sure you want to delete this room: ${room.roomId} ?`)) return
    ...
    async function editRoom(roomId) {
        setErrorallRooms('')
        const data = await roomEdit(roomId,hotelID, typeId, available, price, guests, climate, arrival, starting, services, size)

        if (data.error) return alert(data.error);

        setAllRooms(prev => prev.map(r => r.roomId === roomId ? { ...r,hotelID, typeId, available, price, guests, climate, arrival, starting, services, size } : r))
    ...
    async function handleImageUpload(room) {
        ...
        input.onchange = async (e) => {
            ...
            const data = await uploadRoomImage(room.roomId, formData);
            ...
            if (data.roomImages && data.roomImages.length > 0) {
                const newImageUrls = data.roomImages;

                setAllRooms(prev => 
                    prev.map(r => {
                        if (r.roomId === room.roomId) {
                            // A meglévő 'cityImages' tömbhöz hozzáfűzzük az újakat.
                            const updatedImages = [...r.roomImages, ...newImageUrls];
                            // A 'cityImages' mezőt frissítjük az új, bővített tömbbel.
                            return { ...r, roomImages: updatedImages };
                        }
                        return r;
                    })
                );
                ...
                <div className="modal d-block" tabIndex='-1' style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <div className="modal-dialog">
                        <div className="modal-content p-3">
                            ...
                            <button type="button" className="btn btn-primary mb-2" onClick={() => editRoom(selectedRoom.roomId)}>Modify</button>
